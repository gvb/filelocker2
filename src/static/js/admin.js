Admin = function() {
    ...
    {
        $.getJSON(FILELOCKER_ROOT+'/admin/get_vault_usage?format=json&ms=' + new Date().getTime(), 
        function(returnData)
        {
            if (returnData.fMessages.length !== 0)
                StatusResponse.show(returnData, "updating file vault usage");
    ...
    function loadUsers(length)
    {
        ...
        $.post(FILELOCKER_ROOT+'/admin/get_all_users?format=json', 
        {
    ...
    function createUser()
    {
        ...
        else
        {
            ...
            $.post(FILELOCKER_ROOT+'/admin/create_user?format=json', 
            {
    ...
    //             confirmPassword: $("#createUserPasswordConfirm").val(),
            ...
            function(returnData) 
            {
                StatusResponse.show(returnData, "creating user");
                load(0);
            }, 'json');
        }
    }
    function deleteUsers() 
    {
        ...
        if(userIds !== "")
        {
            $.post(FILELOCKER_ROOT+'/admin/delete_users', 
            {
                "userIds": userIds
            }, 
            function(returnData) {
                StatusResponse.show(returnData, "deleting users");
                load(0);
            }, 'json');
        }
        else
    ...
    function createAttribute()
    {
        ...
        $.post(FILELOCKER_ROOT+'/admin/create_attribute?format=json',
            { "attributeId": attributeId, "attributeName":attributeName }, 
            function(returnData) 
            {
                StatusResponse.show(returnData, "creating attribute");
    ...
    function deleteAttributes()
    {
        ...
        if(attributeIds !== "")
        {
            $.post(FILELOCKER_ROOT+'/admin/delete_attributes?format=json',
                { "attributeIds": attributeIds }, 
                function(returnData) 
                {
                    StatusResponse.show(returnData, "deleting attribute");
                    load(2);
                }, 
                'json');
        }
        else
            StatusResponse.create("deleting attributes", "Select attribute(s) for deletion.", false);
    }
    ...
    function updateConfig()
    {
        ...
        $.post(FILELOCKER_ROOT+'/admin/update_server_config?format=json', values, 
            function(returnData) 
            {
                StatusResponse.show(returnData, "updating config");
                load(3);
            }, 
            'json');
    }
    ...
    {
        $.post(FILELOCKER_ROOT+'/admin/get_user_permissions?format=json', {userId: userId}, 
        function(returnData) 
        {
            if (returnData.fMessages.length === 0)
            {
                ...
                StatusResponse.show(returnData, "getting permissions");
        }, 'json');
    }
    function permissionChecked(userId, permissionId, rowId)
    {
        ...
        {
            $.post(FILELOCKER_ROOT+'/admin/grant_user_permission?format=json', 
            {
                userId: userId,
            ...
            function(returnData) 
            {
                StatusResponse.show(returnData, "granting permissions");
        ...
        {
            $.post(FILELOCKER_ROOT+'/admin/revoke_user_permission?format=json', 
            {
                userId: userId,
                permissionId: permissionId
            }, 
            function(returnData) 
            {
                StatusResponse.show(returnData, "revoking permissions");
                promptUpdatePermissions(userId);
            }, 
            'json');
        }
    ...
    function updateConfigPassword()
    {
        ...
        {
        var configName = 
            $.post(FILELOCKER_ROOT+'/admin/update_config_password?format=json', 
            {
            ...
            function(returnData) 
            {
                StatusResponse.show(returnData, "updating user");
    ...
    function updateFilelockerUser()
    {
        ...
        if ($("#updateUserRole").is(":checked"))
            isRole = "yes";
        $.post(FILELOCKER_ROOT+'/admin/update_filelocker_user?format=json', 
        {
    ...
    function loadTemplateForEditing()
    {
        ...
        if (templateName != "")
        {
            $.post(FILELOCKER_ROOT+'/admin/get_template_text?format=json', 
            {
            ...
            {
                if (returnData.fMessages.length > 0) {
                    StatusResponse.show(returnData, "loading template");
                }
    ...
    function saveTemplate()
    {
        ...
        var templateName = $("#template_selector").val(); 
        $.post(FILELOCKER_ROOT+'/admin/save_template?format=json', 
        {
            templateName: templateName,
        ...
        function(returnData) 
        {
            StatusResponse.show(returnData, "saving template");
    ...
    function revertTemplate()
    {
        var templateText = $("#templateEditArea").val();
        var templateName = $("#template_selector").val(); 
        $.post(FILELOCKER_ROOT+'/admin/revert_template?format=json', 
        {
            templateName: templateName
        }, 
        function(returnData) 
...
jQuery(document).ready(function(){
    bulkUserUploader = new qq.FileUploader({
        ...
        action: FILELOCKER_ROOT+'/admin/bulk_create_user?format=json',
        params: {},
